...
public class HosUserInviteServiceImpl implements IHosUserInviteService{
	...
	@Override
	public CKResponse countObjects(HosUserInviteDto dto) {
		CKResponse cKResponse = new CKResponse();
		int count = 0;
		String errorCode = ConstantApi.ERROR_CODE.SUCCESS;
		String errorMsg = ConstantApi.ERROR_MSG.SUCCESS;
		try {
			count = hosUserInviteMapper.countObjects(dto);
		} catch (SQLException e) {
			logger.error("查询用户邀请信息总数异常:" + e.getMessage());
			errorCode = e.getErrorCode() + "";
			errorMsg = e.getSQLState() + ":" + e.getMessage();
		}
		...
		cKResponse.setErrorCode(errorCode);
		cKResponse.setErrorMsg(errorMsg);
		cKResponse.setObj(count);
		return cKResponse;
	}
	
	@Override
	public CKResponse countNumberInvite(HosUserInviteDto dto) {
		CKResponse cKResponse = new CKResponse();
		Double numberInvite = 0d;
		String errorCode = ConstantApi.ERROR_CODE.SUCCESS;
		String errorMsg = ConstantApi.ERROR_MSG.SUCCESS;
		try {
			numberInvite = hosUserInviteMapper.countNumberInvite(dto);
		} catch (SQLException e) {
			logger.error("查询用户邀请信息总数异常:" + e.getMessage());
			errorCode = e.getErrorCode() + "";
			errorMsg = e.getSQLState() + ":" + e.getMessage();
		...
		cKResponse.setErrorCode(errorCode);
		cKResponse.setErrorMsg(errorMsg);
		cKResponse.setObj(numberInvite);
		return cKResponse;
	}
}
