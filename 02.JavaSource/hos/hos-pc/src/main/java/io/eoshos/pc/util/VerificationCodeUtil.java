...
public class VerificationCodeUtil {
	public static Logger logger = LogManager.getLogger(VerificationCodeUtil.class);

	public static void getCoade(HttpSession session, HttpServletResponse response) throws IOException {
			int width = 95;// 验证码图片宽度
			int height = 47;// 验证码图片高度
			// 创建二进制的输出流
			...
			for (int i = 0; i < length; i++) {
				...
				sRand += ch;
				g.drawString(ch, 25 * i + 3, (random.nextInt(5) - 2) * i + 30);
			}
			// 将生成的字符串存储在session中
